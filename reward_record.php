...
//取得總計獎金
$bonusCountAll = $pdo->query("select SUM(bonus) as bonusAll from `reward_record` where `bonus`")->fetch(PDO::FETCH_ASSOC);
//取得總計發票數量
$invoiceAll = $pdo->query("SELECT COUNT(id) FROM `invoices` WHERE 1")->fetch(PDO::FETCH_ASSOC);

$bc = $boa['SUM(bonus)']; //單期中獎獎金
$rca = $rea['COUNT(id)']; //單期中獎發票總數
$bca = $bonusCountAll['bonusAll']; //總計獎金
$inva = $invoiceAll['COUNT(id)']; //總計發票數;
...
<div class="rightPage h-100 d-flex flex-column justify-content-between">
    ...
    <div class="inv">
        <div class="total col-12 pb-2">
            <div class="col-12 d-flex justify-content-between"><span>本期發票總數：</span><?= $periodCount ?>張</span></div>
            <div class="col-12 d-flex justify-content-between"><span>本期中獎發票：</span><span><?= $rca; ?>張</span></div>
            <div class="col-12 d-flex justify-content-between"><span>本期中獎獎金：</span><span><?= $bc; ?>元</span></div>
            <span class="float-right"><small class="text-danger">(中獎率：<?= $win; ?>%)</small></span>
            <div class="col-12 d-flex justify-content-between border-top"><span>總累計發票數：</span><span><?= $inva; ?>張</span></div>
            <div class="col-12 d-flex justify-content-between"><span>總累計總獎金：</span><span><?= $bca ?>元</span></div>
        </div>
        <?php
        if (!empty($_GET['tip'])) {
        ?>
        ...
        <div class="tit d-flex w-100 py-2 mb-3 border-top">
            <div class="col-5 text-center">發票號碼</div>
            <div class="col-4 text-center">消費日期</div>
            <div class="col-3 text-center">金額</div>
            <div class="text-center d-none d-md-block">獎項</div>
            <div class="text-center d-none d-md-block">獎金</div>
        </div>
        <div class="rewards">
            <?php
            foreach ($rows as $row) {
            ?>
                <div class="reward d-flex flex-wrap">
                    <div class="col-5 text-center"><?= $row['code'] . "-" . $row['number'] ?></div>
                    <div class="col-4 text-center  text-secondary"><?= $row['date'] ?></div>
                    <div class="col-3 text-center"><?= $row['payment'] ?>元</div>
                    <div class="text-center col-3"><?= $row['reward'] ?></div>
                    <div class="text-right col-4 d-block d-md-none mb-4">獎金：</div>
                    <div class="text-right col-5"><?= $row['bonus'] ?>元</div>
                </div>
            <?php
            }
            ?>
