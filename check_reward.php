...
if(empty($inv)){
    header("location:index.php?do=reward_record&y=$year&p=$period&empty=1");
...
($year==$nowYear && ($period<4 || ($period==1 && ($nowPeriod>4 || ($nowMonth==7 && $today>5) || $nowMonth==8)) || ($period==2 && ($nowPeriod>5 || ($nowMonth==9 && $today>5) || $nowMonth==10)) || ($period==3 && (($nowMonth==11 && $today>5) || $nowMonth==12))))){
    header("location:index.php?do=reward_record&y=$year&p=$period&tip=1");
}else{
    header("location:index.php?do=reward_record&y=$year&p=$period");
}
